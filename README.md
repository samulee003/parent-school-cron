...
| `小朋友8歲，最近情緒好大` | 根據年齡和痛點推薦少量相關課程 |
| `更多` / `下一頁` / `還有嗎` | 延續上一個查詢 |
| `同意推送` | 允許之後收到少量主動課程提醒 |
| `暫停推送` / `退訂` / `stop` | 停止主動課程提醒 |
| `私隱` | 查看資料使用和刪除說明 |
| `刪除資料` | 清除該 WhatsApp 家長保存的對話記錄和偏好 |
| `報名` / `報名連結` | 報名方式 |
| `你好` / `help` / `幫助` | 使用說明 |

## Public Beta Safety

公開朋友測試時，請把它說成測試版，不要說成官方服務。分享入口使用：
...
- 課程資料以澳門家長學堂官方網站為準。
- 不用輸入小朋友姓名、學校、證件、住址或其他敏感資料。
- 可以回覆 `私隱` 查看資料說明。
- 可以回覆 `暫停推送` 停止主動提醒。
- 可以回覆 `刪除資料` 清除保存的對話記錄和偏好。
